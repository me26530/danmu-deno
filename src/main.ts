...
Deno.serve(async (request: Request) => {
  ...
  if (url.pathname.endsWith("/__health")) {
    return new Response(
      JSON.stringify({ ok: true, runtime: "deno-deploy", pathname: url.pathname }, null, 2),
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: "Deno Deploy Error", message: error instanceof Error ? error.message : String(error) }, null, 2),
