...
export class DockerRuntimeHandler {
  constructor() {
    throw new Error('Docker runtime is disabled on Supabase Edge Functions');
  }
}
JS
...
export function createDockerEngineClient() {
  throw new Error('Docker engine client is disabled on Supabase Edge Functions');
}
JS
...
export function resolveDockerVersionLookupImage(configuredImageRef = '', runningImageRef = '', defaultRepository = 'lilixu3/danmu-api') {
  return String(configuredImageRef || runningImageRef || defaultRepository || '');
}

export function resolveDockerRuntimeImageTargets({ configuredImageRef = '', runningImageRef = '', defaultRepository = 'lilixu3/danmu-api' } = {}) {
  const image = resolveDockerVersionLookupImage(configuredImageRef, runningImageRef, defaultRepository);
  return {
    configuredImageRef: String(configuredImageRef || ''),
    runningImageRef: String(runningImageRef || ''),
    lookupImageRef: image,
...
export function recordLatestVersion(latestVersion, checkedAt = new Date(), error = '') {
  ...
  state.latestVersionCheckedAt = checkedAt instanceof Date ? checkedAt.toISOString() : String(checkedAt || '');
  state.latestVersionError = error || '';
  return state;
}

export function startRuntimeUpdate(message, targetVersion = '') {
  state.update = { running: false, state: 'disabled', message: message || 'Runtime update is disabled on Supabase Edge Functions', targetVersion, logs: [] };
  return state.update;
}

export function setRuntimeUpdateState(nextState, message = '', targetVersion = '') {
  state.update = { ...state.update, running: false, state: nextState || 'disabled', message, targetVersion };
  return state.update;
}

export function pushRuntimeUpdateLog(message) {
  state.update.logs.push(String(message || ''));
  return state.update;
}

export function setRuntimeUpdateHelperContainer() {
  return state.update;
}

export function finishRuntimeUpdate(success, message, targetVersion = '') {
  state.update = { ...state.update, running: false, state: success ? 'success' : 'failed', message: message || '', targetVersion };
  return state.update;
}
JS
...
export class NodeRuntimeHandler {
  constructor() {
    throw new Error('Node runtime is disabled on Supabase Edge Functions');
  }
}
JS
...
export function resolveCacheFilePathFromModuleUrl(moduleUrl, cacheKey, overridePath = '') {
  return String(overridePath || cacheKey || moduleUrl || '');
}

export function readJsonFileCache() {
  return null;
}

export function writeJsonFileCache() {
  return false;
}
JS
fi
...
if [ -d config ]; then
  rm -rf supabase/functions/danmu/config
  cp -r config supabase/functions/danmu/config
fi
...
test -f supabase/functions/danmu/danmu_api/runtime/runtime-handler-factory.js
test -f supabase/functions/danmu/danmu_api/runtime/docker-engine-client.js
test -f supabase/functions/danmu/danmu_api/runtime/docker-runtime-handler.js
